...
int gr_mesh_cmd(lua_State* L)
{
  ...
  std::vector<Point2D> textcoords;
  std::vector< std::vector <Mesh::Face> > neighbouring_faces;
  std::vector<float> occlusion_factors;
  ...
  bool first_face = true;
	...
	while (m_file.good() && (m_file >> keyword))
	{
		if (keyword == "v")
		{
			float x,y,z;
		...
		else if(keyword == "vt")
		{
			float x, y;
			m_file >> x >> y;
			textcoords.push_back(Point2D(x, y));
		}
		else if(keyword == "vn")
		{
			float x, y, z;
		...
		else if (keyword == "occ")
		{
			float occ;
			m_file >> occ;
			occlusion_factors.push_back(occ);
		}
		else if (keyword == "f")
		{
		//	std::cout<<"in here"<<std::endl;
			if (first_face) {
				  neighbouring_faces.resize(verts.size());
			}
			Mesh::Face face;
			int i = 0;
			...
			while(line.good())
			{
				// this line must only contain
				if (textcoords.size() == 0 && normals.size() == 0)
				{
					line >> vIndex;
					face.vertice_index.push_back(vIndex - 1);
				}
				//the line only contains normal data and vertice data
				else if(textcoords.size() == 0)
				{
					line >> vIndex >> c >> c >> nIndex;
					face.vertice_index.push_back(vIndex - 1);
					face.normal_index.push_back(nIndex - 1);
				}
				else if(normals.size() == 0)
				{
					line >> vIndex >> c >> tIndex;
					face.vertice_index.push_back(vIndex - 1);
					face.texture_index.push_back(tIndex - 1);
				}
				else
				{
			...
			if (!line.good())
			{
//				face.normal_index.pop_back();
//				face.texture_index.pop_back();
//				face.vertice_index.pop_back();
			}
			//calculating area of face
			Vector3D A = Vector3D( (verts[face.vertice_index[0]])[0], (verts[face.vertice_index[0]])[1],
					(verts[face.vertice_index[0]])[2] );
			Vector3D B = Vector3D( (verts[face.vertice_index[1]])[0], (verts[face.vertice_index[1]])[1],
					(verts[face.vertice_index[1]])[2] );
			Vector3D C = Vector3D( (verts[face.vertice_index[2]])[0], (verts[face.vertice_index[2]])[1],
					(verts[face.vertice_index[2]])[2] );
			Vector3D temp = (A-B).cross(A-C);
			double area = temp.length()*0.5;
			face.area = area;

			faces.push_back(face);

			//This code adds the face to all of its vertices
			for (int i = 0; i < face.vertice_index.size(); i++) {
				int index = face.vertice_index[i];
				neighbouring_faces[index].push_back(face);
			}
...
//	std::cout<<textcoords.size()<<std::endl;
	Mesh* mesh = new Mesh(verts, faces, normals, textcoords, neighbouring_faces, occlusion_factors);
	GRLUA_DEBUG(*mesh);
